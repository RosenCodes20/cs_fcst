...
class Menu
{
	function get($id)
	{
		return MySQL::get("SELECT * FROM menus p WHERE p.id = '$id' AND p.active = 'Yes' AND p.language_id = '".($_SESSION['language']->id)."'");
	}
	...
	function search($criteria = false, &$navigation = false)
	{
		...
		$a = "AND p.active = 'Yes' AND p.language_id = '".($_SESSION['language']->id)."'";
